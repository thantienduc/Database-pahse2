...
def insert():
    ...
    if form.validate_on_submit():
        ...
        sid = form.SId.data
        cur = cnx.cursor()
        cur.execute('INSERT INTO Instructor (ID, FName, LName, StartDate, Degree, Rank, Type) VALUES(%s,%s,%s,%s,%s,%s,%s)',
...
def update():
    ...
    if form.validate_on_submit():
        ...
        cur = cnx.cursor()
        cur.execute("UPDATE Instructor SET FName= %s, LName = %s WHERE ID =%s", (fname, lname, studentId) )
        cnx.commit()
        cur.close()
